...
|---|---|---|
| `download_datasets.py` | Fetches SisFall + UMAFall from the repo's `datasets-v1` release (upstream fallback for UMAFall), verifies against `datasets/SHA256SUMS.txt`, optionally extracts into the parser-expected layout; prints manual instructions for FallAllD (DataPort login) and KFall (registration; never redistributable). | Reproducible, checksummed data acquisition. |
