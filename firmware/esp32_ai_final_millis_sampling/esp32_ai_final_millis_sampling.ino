...
void setup() {
  ...
  // Stable 50 Hz polling for Arduino ESP32 core compatibility.
  gLastSampleMs = millis();
  Serial.println("sampling: millis polling at 50 Hz");
...
void loop() {
  ...
  if (gAlarmActive && (now - gAlarmStartMs >= kAlarmHoldMs)) {
    stopAlarm();
  }

  // Stable 50 Hz sampling without the version-dependent timer API.
  if (now - gLastSampleMs < (1000 / kSampleRateHz)) {
    return;
  }
  gLastSampleMs = now;
  ...
  if (readSensorRaw(s)) {
    ...
    const float ax = s[0] * kAccelScale;
    const float ay = s[1] * kAccelScale;
    const float az = s[2] * kAccelScale;
    const float gx = s[3] * kGyroScale;
    const float gy = s[4] * kGyroScale;
    const float gz = s[5] * kGyroScale;
  ...
  if (gNewSamples >= (uint32_t)kStride &&
      gWriteIdx >= (uint32_t)kWindowLen) {
    gNewSamples = 0;
    runInference();
